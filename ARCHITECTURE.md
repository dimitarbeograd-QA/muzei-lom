...
Активният темплейт се пази сайт-широко (не per-visitor) в
`localStorage['mlcms_template']`, огледално на другия admin-управляван
контент (`getData`/`setData`, `mlcms_` префикс). Прилага се възможно
най-рано чрез малък inline `<script>` веднага след `<body>`, за да няма
"проблясване" на грешна тема при зареждане.
